#include <merak/session_store_pg.hpp>
...
SessionStorePG::SessionStorePG(std::shared_ptr<pqxx::connection> conn)
    : conn_(std::move(conn)) {}

SessionStorePG::~SessionStorePG() = default;

void SessionStorePG::exec(const std::string& sql) {
    pqxx::work txn(*conn_);
    txn.exec(sql);
    txn.commit();
}

void SessionStorePG::initialize() {
...
SessionRecord SessionStorePG::create_session(const std::string& title,
...
void SessionStorePG::update_session(const std::string& id, const std::string& title) {
    std::lock_guard lock(mutex_);
    auto ts = now_iso();
...
SessionRecord SessionStorePG::archive_session(const std::string& id, bool archived) {
    std::lock_guard lock(mutex_);
    auto ts = now_iso();
...
std::vector<SessionRecord> SessionStorePG::list_sessions(const std::string& world_id) const {
    std::lock_guard lock(mutex_);
...
RunRecord SessionStorePG::create_run(
...
std::optional<RunRecord> SessionStorePG::get_run(const std::string& id) const {
    std::lock_guard lock(mutex_);
...
bool SessionStorePG::has_unfinished_run(const std::string& session_id) const {
    std::lock_guard lock(mutex_);
...
void SessionStorePG::update_run_status(const std::string& id, RunStatus status,
...
ApprovalRecord SessionStorePG::create_approval(ApprovalRecord approval) {
    std::lock_guard lock(mutex_);
...
std::optional<ApprovalRecord> SessionStorePG::get_approval(const std::string& id) const {
    std::lock_guard lock(mutex_);
...
ApprovalRecord SessionStorePG::resolve_approval(const std::string& id, ApprovalStatus status) {
...
RuntimeEvent SessionStorePG::append_event(RuntimeEvent event) {
    std::lock_guard lock(mutex_);
...
std::vector<RuntimeEvent> SessionStorePG::events_after(const std::string& session_id,
...
std::vector<RunRecord> SessionStorePG::interrupt_running_runs() {
    std::lock_guard lock(mutex_);
...
void SessionStorePG::save_checkpoint(const std::string& id, const std::string& run_id,
...
std::optional<std::string> SessionStorePG::load_latest_checkpoint_json(const std::string& run_id) {
    std::lock_guard lock(mutex_);
...
std::vector<std::string> SessionStorePG::list_checkpoints_json(const std::string& run_id) {
    std::lock_guard lock(mutex_);
...
void SessionStorePG::prune_checkpoints(const std::string& run_id, int keep_latest_n) {
    std::lock_guard lock(mutex_);
