...
> 基于 `main` 分支源码严格生成，2026-06-19（更新：P0 后端补全新增 30 个端点）。
...
| `knowledge_not_found` | 404 | 知识条目不存在 |
| `faction_not_found` | 404 | 势力不存在 |
| `file_link_not_found` | 404 | 文件关联不存在 |
| `creation_not_found` | 404 | 待定创建不存在 |
| `unknown_tool` | 400 | 不支持的 tool_name |
...
#### `DELETE /api/worldbuilding/:wid/agents/:aid` — 删除角色

删除角色及其全部关联数据（角色卡、日记、关系）。
...
#### `GET /api/worldbuilding/:wid/agents/:aid/diaries/:did` — 获取单篇日记
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `diary.id` | string | 日记 ID |
| `diary.agent_id` | string | Agent ID |
| `diary.scene_id` | string | 关联场景 ID |
| `diary.world_time` | string | 世界时间 |
| `diary.content` | string | 日记内容 |
| `diary.mood` | string | 心情 |
| `diary.status` | string | 状态 |
| `diary.leak_risk_level` | number | 泄露风险等级 |
| `diary.tokens_used` | number | token 用量 |
| `diary.created_at` | string | 创建时间 |

**`404`**：`agent_not_found` — Agent 不存在或不属于该世界；日记不存在

---

#### `PATCH /api/worldbuilding/:wid/agents/:aid/diaries/:did` — 更新日记
...
**`400`**：`invalid_request` — JSON 解析失败或缺少必填字段
**`404`**：`agent_not_found` — Agent 不存在或不属于该世界

---

#### `GET /api/worldbuilding/:wid/agents/:aid/diaries/search` — 搜索日记
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `diaries` | array | 匹配的日记列表 |
| `total` | number | 总匹配数 |

**`400`**：`invalid_request` — 缺少 q 参数
**`404`**：`agent_not_found`

---

#### `GET /api/worldbuilding/:wid/agents/:aid/memory-summaries` — 列出记忆摘要
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `summaries` | array | 记忆摘要列表 |
| `summaries[].id` | string | 摘要 ID |
| `summaries[].period_start` | string | 周期起始时间 |
| `summaries[].period_end` | string | 周期结束时间 |
| `summaries[].summary` | string | 摘要内容 |
| `summaries[].source_diary_ids` | string[] | 来源日记 ID |
| `summaries[].created_at` | string | 创建时间 |

**`404`**：`agent_not_found`

---

#### `GET /api/worldbuilding/:wid/agents/:aid/memory-summaries/:mid` — 获取单个记忆摘要
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `summary.id` | string | 摘要 ID |
| `summary.period_start` | string | 周期起始时间 |
| `summary.period_end` | string | 周期结束时间 |
| `summary.summary` | string | 摘要内容 |
| `summary.source_diary_ids` | string[] | 来源日记 ID |
| `summary.created_at` | string | 创建时间 |

**`404`**：`agent_not_found`；摘要不存在
...
#### `POST /api/worldbuilding/:wid/agents/:aid/relations` — 创建或更新角色关系

通过 upsert 机制，同一对角色只保留一条关系记录。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `target_id` | string | 是 | 目标角色 ID |
| `relation_type` | string | 否 | 关系类型，默认空字符串 |
| `description` | string | 否 | 关系描述，默认空字符串 |
| `intimacy` | number | 否 | 亲密度 (0-100)，默认 0 |

**响应 `201`**：`{"ok": true}`

**`400`**：`invalid_request` — JSON 解析失败或缺少 target_id
**`404`**：`agent_not_found`

---

#### `PATCH /api/worldbuilding/:wid/agents/:aid/relations/:tid` — 更新角色关系
...
#### `GET /api/worldbuilding/:wid/agents/:aid/voice` — 获取角色声音指纹
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `voice.avg_sentence_length` | number | 平均句子长度 |
| `voice.sentence_variance` | number | 句子长度方差 |
| `voice.question_frequency` | number | 问句频率 |
| `voice.modifier_ratio` | number | 修饰词比率 |
| `voice.sample_count` | number | 分析样本数量 |
| `voice.signature_words` | string[] | 标志性用词 |
| `voice.tone_profile` | object | 语气特征 |
...
#### `GET /api/worldbuilding/:wid/tree` — 章节目录树

返回按弧线组织的章节目录树，用于侧边栏导航。未归类的章节归入 `"Uncategorized"` 节点（`id` 为空字符串）。
...
| `ok` | boolean | 操作是否成功 |
| `tree` | array | 弧线节点列表 |
| `tree[].id` | string | 弧线 ID（未归类为空字符串） |
| `tree[].title` | string | 弧线标题 |
| `tree[].status` | string | 弧线状态 |
| `tree[].chapters` | array | 该弧线下的章节列表（按编号排序） |
| `tree[].chapters[].id` | string | 章节 ID |
| `tree[].chapters[].title` | string | 章节标题 |
| `tree[].chapters[].number` | number | 章节编号 |
| `tree[].chapters[].status` | string | 章节状态 |
| `tree[].chapters[].scene_count` | number | 场景数量 |
| `tree[].chapters[].updated_at` | string | 更新时间 |
| `tree[].chapters[].arc_id` | string\|null | 关联弧线 ID |
...
#### `DELETE /api/worldbuilding/:wid/chapters/:cid` — 删除章节

同时清理关联的场景和父弧线的 `chapter_numbers` 引用。
...
#### `POST /api/worldbuilding/:wid/chapters/reorder` — 重新排序章节
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `order` | string[] | 是 | 按新顺序排列的章节 ID 列表 |

**响应 `200`**：`{"ok": true}`

**`400`**：`invalid_request` — 排序失败或内部异常
...
#### `GET /api/worldbuilding/:wid/scenes/:sid` — 获取单个场景
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `scene.id` | string | 场景 ID |
| `scene.title` | string | 场景标题 |
| `scene.chapter_id` | string | 所属章节 ID |
| `scene.world_time` | string | 世界时间 |
| `scene.narrative` | string | 叙述内容 |
| `scene.status` | string | 状态：`"draft"` / `"writing"` / `"completed"` |
| `scene.participant_ids` | string[] | 参与角色 ID |
| `scene.plot_goal` | string | 情节目标 |
| `scene.emotional_goal` | string | 情感目标 |
| `scene.information_goal` | string | 信息目标 |
| `scene.external_conflict` | string | 外部冲突 |
| `scene.internal_conflict` | string | 内部冲突 |
| `scene.foreshadowing_ids` | string[] | 关联伏笔 ID |
| `scene.style_overrides` | object | 风格覆盖 |
| `scene.section_id` | string\|null | 关联区域 ID |
| `scene.location_id` | string\|null | 关联地点 ID |
| `scene.pov_character_id` | string\|null | 视角角色 ID |
| `scene.hidden_conflict` | string\|null | 隐藏冲突 |
...
#### `DELETE /api/worldbuilding/:wid/scenes/:sid` — 删除场景

同时清理关联父章节和区域的 `scene_ids` 引用。
...
#### `DELETE /api/worldbuilding/:wid/foreshadowing/:id` — 删除伏笔
...
#### `DELETE /api/worldbuilding/:wid/secrets/:id` — 删除秘密
...
#### `POST /api/worldbuilding/:wid/export-full` — 导出世界完整快照

将世界所有数据导出为单一 JSON 快照（`share-snapshot-v1` 格式），用于备份或分享。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `include_diaries` | boolean | 否 | 是否包含角色日记，默认 false |
| `include_memories` | boolean | 否 | 是否包含记忆摘要，默认 false |
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `snapshot.schema_version` | string | 快照格式版本（`"1.0"`） |
| `snapshot.exported_at` | string | 导出时间 |
| `snapshot.snapshot_id` | string | 快照唯一标识 |
| `snapshot.source.world_id` | string | 源世界 ID |
| `snapshot.source.name` | string | 源世界名称 |
| `snapshot.manifest` | object | 清单（agent_count、chapter_count、scene_count 等） |
| `snapshot.payload` | object | 全量数据（world、agents、chapters、scenes、arcs、locations、factions、knowledge、foreshadowing、secrets、timeline、images） |

> 图片处理：≤256KB 的图片 base64 内嵌在 `data` 字段；更大的图片 `data` 为 null 并标记 `"transfer": "external"`。
...
#### `POST /api/worldbuilding/import` — 导入世界快照

从 JSON 快照创建新世界，所有 ID 重映射。注意：此路径不含 `:wid`。

**请求体**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `snapshot` | object | 是 | 快照对象（export-full 产出的完整 JSON） |
| `target_name` | string | 否 | 目标世界名称，默认使用源世界名称 |
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `world_id` | string | 新创建的世界 ID |
| `id_mapping` | object | 旧 ID → 新 ID 的映射表 |

> ID 重映射覆盖所有交叉引用字段（`participant_ids`、`scene_ids`、`member_agent_ids`、`rival_faction_ids`、`foreshadowing_ids`、`related_ids` 等）。`fork_chain` 数组记录派生链。内嵌的 base64 图片写入本地 ImageService，>256KB 的大图跳过。

**`400`**：`invalid_request` — 快照解析失败
**`500`**：`export_failed`
...
#### `GET /api/worldbuilding/:wid/locations` — 列出地点
...
| `locations[].region` | string | 所属区域 |
| `locations[].parent_location_id` | string\|null | 父地点 ID |
| `locations[].created_at` | string | 创建时间 |

---

#### `GET /api/worldbuilding/:wid/locations/:lid` — 获取单个地点
...
**响应 `200`**：同列表中的 location 对象。

**`404`**：`location_not_found`

---

#### `POST /api/worldbuilding/:wid/locations` — 创建地点
...
|------|------|------|------|
| `name` | string | 是 | 地点名称 |
| `description` | string | 否 | 地点描述，默认空字符串 |
| `region` | string | 否 | 所属区域，默认空字符串 |
| `parent_location_id` | string | 否 | 父地点 ID |

**响应 `201`**：`{"ok": true, "location_id": "<id>"}`

**`400`**：`invalid_request` — 缺少 name

---

#### `PATCH /api/worldbuilding/:wid/locations/:lid` — 更新地点
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `fields` | object | 是 | 要更新的字段（name, description, region, parent_location_id） |
...
#### `DELETE /api/worldbuilding/:wid/locations/:lid` — 删除地点
...
#### `GET /api/worldbuilding/:wid/knowledge` — 列出知识条目
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `category` | string | 否 | 按类别过滤，默认返回全部 |
...
| `knowledge[].id` | string | 条目 ID |
| `knowledge[].category` | string | 类别 |
| `knowledge[].content` | string | 内容 |
| `knowledge[].tags` | string[] | 标签 |
| `knowledge[].aliases` | string[] | 别名 |
| `knowledge[].related_ids` | string[] | 关联条目 ID |
| `knowledge[].created_at` | string | 创建时间 |

---

#### `POST /api/worldbuilding/:wid/knowledge` — 创建知识条目
...
| `content` | string | 是 | 内容 |
| `tags` | string[] | 否 | 标签，默认空数组 |
| `aliases` | string[] | 否 | 别名，默认空数组 |
| `related_ids` | string[] | 否 | 关联条目 ID，默认空数组 |
...
#### `PATCH /api/worldbuilding/:wid/knowledge/:kid` — 更新知识条目
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `fields` | object | 是 | 要更新的字段（category, content, tags, aliases, related_ids） |
...
#### `DELETE /api/worldbuilding/:wid/knowledge/:kid` — 删除知识条目
...
#### `GET /api/worldbuilding/:wid/factions` — 列出势力
...
| `ok` | boolean | 操作是否成功 |
| `factions` | array | 势力列表 |
| `factions[].id` | string | 势力 ID |
| `factions[].world_id` | string | 所属世界 ID |
| `factions[].name` | string | 势力名称 |
| `factions[].description` | string | 势力描述 |
| `factions[].goals` | string | 势力目标 |
| `factions[].member_agent_ids` | string[] | 成员角色 ID |
| `factions[].rival_faction_ids` | string[] | 敌对势力 ID |
| `factions[].created_at` | string | 创建时间 |
| `factions[].updated_at` | string | 更新时间 |

---

#### `GET /api/worldbuilding/:wid/factions/:fid` — 获取单个势力
...
**响应 `200`**：同列表中的 faction 对象。

**`404`**：`faction_not_found`

---

#### `POST /api/worldbuilding/:wid/factions` — 创建势力
...
|------|------|------|------|
| `name` | string | 是 | 势力名称 |
| `description` | string | 否 | 势力描述，默认空字符串 |
| `goals` | string | 否 | 势力目标，默认空字符串 |
| `member_agent_ids` | string[] | 否 | 成员角色 ID，默认空数组 |
| `rival_faction_ids` | string[] | 否 | 敌对势力 ID，默认空数组 |

**响应 `201`**：`{"ok": true, "faction_id": "<id>"}`

**`400`**：`invalid_request` — 缺少 name

---

#### `PATCH /api/worldbuilding/:wid/factions/:fid` — 更新势力
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `fields` | object | 是 | 要更新的字段（name, description, goals, member_agent_ids, rival_faction_ids） |
...
#### `DELETE /api/worldbuilding/:wid/factions/:fid` — 删除势力
...
#### `POST /api/worldbuilding/:wid/dashboard` — 世界创作仪表盘

聚合世界所有维度的统计数据，用于创作进度总览。
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `dashboard.world` | object | 世界基本信息（id, name, description, created_at, updated_at） |
| `dashboard.agents` | object | Agent 统计：`total`（总数）、`by_kind`（按类型分布） |
| `dashboard.chapters` | object | 章节统计：`total`, `completed`, `drafting`, `outline`, `revised` |
| `dashboard.scenes` | object | 场景统计：`total`, `completed`, `writing`, `draft` |
| `dashboard.arcs` | object | 弧线：`total`（总数）、`items`（列表） |
| `dashboard.locations` | object | 地点：`total` |
| `dashboard.knowledge` | object | 知识条目：`total`、`by_category`（按类别计数） |
| `dashboard.factions` | object | 势力：`total` |
| `dashboard.secrets` | object | 秘密统计：`active`, `exposed`, `abandoned`, `total` |
| `dashboard.foreshadowing` | object | 伏笔统计：`open`, `paid`, `abandoned` |
| `dashboard.file_links` | object | 文件关联：`total` |
| `dashboard.reminders` | array | 待处理提醒列表（含 `type`, `id`, `content`/`holder_id` 等） |
| `dashboard.progress` | object | 进度百分比：`chapter_completion_pct`, `scene_completion_pct`, `foreshadowing_paid_pct`, `secret_exposed_pct` |
...
#### `GET /api/worldbuilding/:wid/files` — 列出文件关联
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `files` | array | 文件关联列表（由 `list_file_links` 返回的原始 JSON） |

---

#### `POST /api/worldbuilding/:wid/files` — 创建文件关联
...
| `target_type` | string | 是 | 关联目标类型 |
| `target_id` | string | 是 | 关联目标 ID |
...
#### `DELETE /api/worldbuilding/:wid/files/:file_path` — 删除文件关联
...
**`404`**：`file_link_not_found`

---

### 4.14 Agent 驱动的预览与创建

Agent 在执行创建操作时，先生成预览供用户确认，确认后存储为待定创建，最终由用户决定 allow/modify/deny。

#### `POST /api/worldbuilding/:wid/previews/:tool_name` — 构建创建预览
...
|------|------|
| `wid` | 世界 ID |
| `tool_name` | 工具名：`create_scene` / `create_chapter` / `create_arc` / `create_secret` / `add_world_knowledge` / `add_location` |

**请求体**：以 `params` 字段嵌套或与顶层平铺的参数对象。
...
| `preview` | object | 预览内容（结构因 tool_name 而异，含建议的字段值） |

**`400`**：`unknown_tool` — 不支持的 tool_name

---

#### `POST /api/worldbuilding/:wid/creations` — 存储待定创建
...
| `params` | object | 是 | 创建参数 |
| `preview` | object | 是 | 预览内容 |

**响应 `201`**：`{"ok": true, "creation_id": "<id>"}`

**`400`**：`invalid_request` — 缺少必填字段

---

#### `GET /api/worldbuilding/:wid/creations/:creation_id` — 获取待定创建
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `creation.creation_id` | string | 创建请求 ID |
| `creation.tool_name` | string | 工具名 |
| `creation.world_id` | string | 世界 ID |
| `creation.params` | object | 创建参数 |
| `creation.preview` | object | 预览内容 |

**`404`**：`creation_not_found`

---

#### `POST /api/worldbuilding/:wid/creations/:creation_id/resolve` — 处理创建请求
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `decision` | string | 是 | `"allow"` / `"modify"` / `"deny"` |
| `modifications` | object | 否 | 修改内容（`decision="modify"` 时使用），默认空对象 |

**响应 `200`**：
- `deny`：`{"ok": true, "decision": "deny", "creation_id": "<id>"}`
- `allow` / `modify`：`{"ok": true, "result": {...}}`，result 中根据 tool_name 包含 `scene_id` / `chapter_id` / `arc_id` / `secret_id` 等
...
#### `POST /api/worldbuilding/:wid/pipeline/retreat` — 回退 Pipeline 阶段

回退到指定阶段，重置目标阶段的所有条件状态。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `to_phase` | string | 是 | 目标阶段（如 `"outlining"`、`"drafting"`、`"revising"`、`"polishing"`） |
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `state` | object | 更新后的 Pipeline 状态（同 state 接口响应） |
...
#### `POST /api/worldbuilding/:wid/pipeline/clear-error` — 清除 Pipeline 错误
...
**`400`**：清除失败，返回 `{"ok": false, "error": "<失败原因>"}`
**`503`**：Pipeline Manager 未初始化
...
用于查询世界知识图谱中的实体、关系及场景抽取结果。注意：此处为只读查询端点，extraction 由 GodAgent 在对话中完成。

#### `GET /api/worldbuilding/:wid/knowledge-graph/entities` — 列出 KG 实体
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `type` | string | 否 | 实体类型过滤：`"agent"`、`"location"`、`"faction"`、`"knowledge"`，默认返回全部 |
...
| `ok` | boolean | 操作是否成功 |
| `entities` | array | 实体列表 |
| `entities[].id` | string | 实体 ID |
| `entities[].name` | string | 实体名称 |
| `entities[].type` | string | 实体类型 |
| `entities[].properties` | object | 实体属性 |

**`404`**：`world_not_found`

---

#### `GET /api/worldbuilding/:wid/knowledge-graph/entities/:eid/relations` — 获取实体关系
...
| `relations` | array | 关系列表 |
| `relations[].relation_type` | string | 关系类型 |
| `relations[].target_id` | string | 目标实体 ID |
| `relations[].target_name` | string | 目标实体名称 |
| `relations[].target_type` | string | 目标实体类型 |

**`404`**：`world_not_found`；实体不存在

---

#### `GET /api/worldbuilding/:wid/knowledge-graph/search` — 搜索 KG 实体
...
| `results[].id` | string | 实体 ID |
| `results[].name` | string | 实体名称 |
...
#### `GET /api/worldbuilding/:wid/scenes/:sid/extraction-result` — 获取场景抽取结果

返回某场戏的 KG extraction 候选结果。
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `candidates` | array | 候选关系列表 |
| `candidates[].relation.subject_id` | string | 主体 ID |
| `candidates[].relation.predicate` | string | 谓语/关系 |
| `candidates[].relation.object_id` | string | 客体 ID |
| `candidates[].status` | string | 候选状态 |
| `candidates[].evidence` | string | 证据文本 |
| `candidates[].change_summary` | string | 变更摘要 |
...
世界时间线管理，包括事件记录和世界时间推进。

#### `GET /api/worldbuilding/:wid/timeline` — 获取时间线
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `current_time.day` | number | 当前天数 |
| `current_time.period` | number | 当前时段（0=晨, 1=昼, 2=午, 3=晚, 4=夜） |
| `current_time.label` | string | 当前时间可读标签 |
| `events` | array | 时间线事件列表 |
| `events[].id` | string | 事件 ID |
| `events[].world_time` | string | 世界时间 |
| `events[].description` | string | 事件描述 |
| `events[].recorded_by` | string | 记录者 ID |
| `events[].affected_character_ids` | string[] | 受影响角色 ID |
| `events[].related_scene_ids` | string[] | 关联场景 ID |

**`404`**：`world_not_found`

---

#### `GET /api/worldbuilding/:wid/timeline/events/:eid` — 获取单个时间线事件
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `event.id` | string | 事件 ID |
| `event.world_time` | string | 世界时间 |
| `event.description` | string | 事件描述 |
| `event.recorded_by` | string | 记录者 ID |
| `event.recorded_by_name` | string | 记录者名称 |
| `event.affected_character_ids` | string[] | 受影响角色 ID |
| `event.related_scene_ids` | string[] | 关联场景 ID |

**`404`**：`world_not_found`；事件不存在

---

#### `POST /api/worldbuilding/:wid/timeline` — 手动记录时间线事件

记录任意世界时间点的事件，不校验"只能向前"——支持回忆/闪回/补录。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `world_time` | string | 是 | 事件发生的世界时间（如 `"第2日午"`） |
| `description` | string | 是 | 事件描述 |
| `recorded_by` | string | 否 | 记录者，默认 `"user"` |
| `affected_character_ids` | string[] | 否 | 受影响的角色 ID 列表 |
| `related_scene_ids` | string[] | 否 | 关联场景 ID 列表 |
...
| `event.world_time` | string | 世界时间 |
| `event.description` | string | 事件描述 |
| `event.recorded_by` | string | 记录者 |
| `event.affected_character_ids` | string[] | 受影响的角色 ID |
| `event.related_scene_ids` | string[] | 关联场景 ID |

**`400`**：`invalid_request` — 缺少必填字段或 JSON 解析失败
**`404`**：`world_not_found`

> **与 `POST /timeline/advance` 的区别**：`/timeline` 不校验时间先后，可记录过去/未来的任意事件；`/timeline/advance` 要求新时间严格晚于当前世界时间。

---

#### `POST /api/worldbuilding/:wid/timeline/advance` — 推进世界时间

推进世界时间并可选记录时间线事件。替代旧的 `/api/worldbuilding/:wid/time/advance` 端点。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `time_label` | string | 是 | 时间描述，支持中英文格式和 duration 简写（如 `"第3日晚"`、`"day3_evening"`、`"2d"`、`"4h"`） |
| `description` | string | 否 | 事件描述 |
| `recorded_by` | string | 否 | 记录者 ID |
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `new_time.day` | number | 新天数 |
| `new_time.period` | number | 新时段 |
| `new_time.label` | string | 新时间可读标签 |

**`400`**：`time_not_forward` — 新时间不晚于当前世界时间
**`404`**：`world_not_found`
...
| AgentKind | `god`, `map_manager`, `history_manager`, `magic_system_manager`, `faction_manager`, `relation_manager`, `individual`, `group`, `writer` |
...
| DELETE | `/api/worldbuilding/:wid/agents/:aid` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/diaries` | Agent |
| POST | `/api/worldbuilding/:wid/agents/:aid/diaries` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/diaries/:did` | Agent |
| PATCH | `/api/worldbuilding/:wid/agents/:aid/diaries/:did` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/diaries/search` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/memory-summaries` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/memory-summaries/:mid` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/relations` | Agent |
| POST | `/api/worldbuilding/:wid/agents/:aid/relations` | Agent |
| PATCH | `/api/worldbuilding/:wid/agents/:aid/relations/:tid` | Agent |
| GET | `/api/worldbuilding/:wid/agents/:aid/voice` | Agent |
...
| GET | `/api/worldbuilding/:wid/tree` | 叙事 |
...
| DELETE | `/api/worldbuilding/:wid/chapters/:cid` | 叙事 |
| POST | `/api/worldbuilding/:wid/chapters/reorder` | 叙事 |
| GET | `/api/worldbuilding/:wid/chapters/:cid/review` | 叙事 |
| GET | `/api/worldbuilding/:wid/scenes` | 叙事 |
| GET | `/api/worldbuilding/:wid/scenes/:sid` | 叙事 |
| POST | `/api/worldbuilding/:wid/scenes` | 叙事 |
| PATCH | `/api/worldbuilding/:wid/scenes/:sid` | 叙事 |
| DELETE | `/api/worldbuilding/:wid/scenes/:sid` | 叙事 |
...
| DELETE | `/api/worldbuilding/:wid/foreshadowing/:id` | 伏笔 |
| GET | `/api/worldbuilding/:wid/secrets` | 秘密 |
| POST | `/api/worldbuilding/:wid/secrets` | 秘密 |
| PATCH | `/api/worldbuilding/:wid/secrets/:id` | 秘密 |
| DELETE | `/api/worldbuilding/:wid/secrets/:id` | 秘密 |
| POST | `/api/worldbuilding/:wid/export` | 导出 |
| POST | `/api/worldbuilding/:wid/export-full` | 导出 |
| POST | `/api/worldbuilding/import` | 导出 |
| GET | `/api/worldbuilding/:wid/locations` | 地点 |
| GET | `/api/worldbuilding/:wid/locations/:lid` | 地点 |
| POST | `/api/worldbuilding/:wid/locations` | 地点 |
| PATCH | `/api/worldbuilding/:wid/locations/:lid` | 地点 |
| DELETE | `/api/worldbuilding/:wid/locations/:lid` | 地点 |
| GET | `/api/worldbuilding/:wid/knowledge` | 知识 |
| POST | `/api/worldbuilding/:wid/knowledge` | 知识 |
| PATCH | `/api/worldbuilding/:wid/knowledge/:kid` | 知识 |
| DELETE | `/api/worldbuilding/:wid/knowledge/:kid` | 知识 |
| GET | `/api/worldbuilding/:wid/factions` | 势力 |
| GET | `/api/worldbuilding/:wid/factions/:fid` | 势力 |
| POST | `/api/worldbuilding/:wid/factions` | 势力 |
| PATCH | `/api/worldbuilding/:wid/factions/:fid` | 势力 |
| DELETE | `/api/worldbuilding/:wid/factions/:fid` | 势力 |
| POST | `/api/worldbuilding/:wid/dashboard` | 仪表盘 |
| GET | `/api/worldbuilding/:wid/files` | 文件关联 |
| POST | `/api/worldbuilding/:wid/files` | 文件关联 |
| DELETE | `/api/worldbuilding/:wid/files/:file_path` | 文件关联 |
| POST | `/api/worldbuilding/:wid/previews/:tool_name` | Agent 驱动 |
| POST | `/api/worldbuilding/:wid/creations` | Agent 驱动 |
| GET | `/api/worldbuilding/:wid/creations/:creation_id` | Agent 驱动 |
| POST | `/api/worldbuilding/:wid/creations/:creation_id/resolve` | Agent 驱动 |
...
| POST | `/api/worldbuilding/:wid/pipeline/retreat` | Pipeline |
| POST | `/api/worldbuilding/:wid/pipeline/clear-error` | Pipeline |
| GET | `/api/worldbuilding/:wid/knowledge-graph/entities` | KG |
| GET | `/api/worldbuilding/:wid/knowledge-graph/entities/:eid/relations` | KG |
| GET | `/api/worldbuilding/:wid/knowledge-graph/search` | KG |
| GET | `/api/worldbuilding/:wid/scenes/:sid/extraction-result` | KG |
| GET | `/api/worldbuilding/:wid/timeline` | Timeline |
| GET | `/api/worldbuilding/:wid/timeline/events/:eid` | Timeline |
| POST | `/api/worldbuilding/:wid/timeline/advance` | Timeline |
| POST | `/api/worldbuilding/:wid/timeline` | Timeline |

**总计：124 个 API 端点**（28 core + 96 worldbuilding），加 1 个 SSE 流端点，加 1 个静态文件挂载（`/`）。
