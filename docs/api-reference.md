...
> **已知不一致**：`PATCH /api/worldbuilding/worlds/:id` 在 404 时返回 `{"error": "..."}` 而非标准 worldbuilding 错误格式。
...
| `modifications` | object | 否 | 对创建内容的修改建议，默认空对象 |
...
### 1.17 `GET /v1/runs/:id` — 获取运行详情

从事件流中聚合工具调用和 token 用量信息。会扫描该 run 的所有 `tool_started`/`tool_completed` 及其 sub_run 变体事件，以及 `usage_updated`/`sub_run_usage_updated` 事件来累加 token 计数。
...
| `run.model` | string | 使用的模型（取自 metadata，非 run 自身字段） |
| `run.started_at` | string | 开始时间 |
| `run.completed_at` | string\|null | 完成时间（未完成时为 null） |
| `run.input_tokens` | number | 输入 token 总数（累加 usage_updated 事件） |
| `run.output_tokens` | number | 输出 token 总数（累加 usage_updated 事件） |
| `run.tool_calls` | array | 工具调用记录（从 tool_started/completed 事件聚合） |
...
### 1.18 `GET /v1/sessions/:id/events/stream` — SSE 事件流
...
| `creation_requested` | — | Agent 请求创建资源，需要用户确认 |
| `creation_resolved` | — | 创建请求已处理 |
...
| event | payload 字段 | 说明 |
|-------|-------------|------|
| `session_created` | `title` | 会话创建（不在 SSE 流中消费） |
| `session_updated` | `title` | 会话标题已更新（不在 SSE 流中消费） |
...
从服务器缓存的配置中返回（服务器启动时从 `ConfigLoader::load()` 加载）。
...
**`500`**：`config_load_failed` — 缓存配置为 null（未加载）

---

### 2.2 `POST /api/config/llm` — 保存 LLM 配置

写入 `{merak_home}/settings.local.json`，与已有配置合并（不会覆盖未传入的字段）。
...
| `files[].mime` | string | MIME 类型（text/markdown, application/json, application/yaml, text/plain） |
...
| `session_id` | string | 否 | 关联会话（用于 SSE 广播，空字符串不广播） |
...
| `agent.images.avatar` | array | 头像图片列表（含 `id`, `url`, `is_primary`, `mime_type`, `file_size_bytes`, `original_name`, `sort_order`, `created_at`） |
| `agent.images.design` | array | 人设图列表（结构同 avatar） |
...
角色图片分为两种类型：`avatar`（头像，每个角色一个主头像）和 `design`（人设图，每个角色多张）。图片仅供前端展示，Agent 不会读取。
...
| `images[].mime_type` | string | MIME 类型（如 `image/png`） |
| `images[].original_name` | string | 原始文件名 |
| `images[].file_size_bytes` | number | 文件大小（字节） |
| `images[].is_primary` | boolean | 是否为主头像（同一 image_type 下只有一个为 true） |
...
适用于小于 10 MB 的文件。大于 10 MB 自动走分块上传。
...
| `uploaded_chunks` | number[] | 已上传的所有块索引列表（用于断点续传） |
...
当 Image Service 可用时，Agent 列表和详情接口会自动附加图片信息：

**`GET /api/worldbuilding/:wid/agents` 列表**中每个 agent 增加：

| 字段 | 类型 | 说明 |
|------|------|------|
| `avatar_url` | string | 主头像 URL（无头像时不存在此字段） |

**`GET /api/worldbuilding/:wid/agents/:aid` 详情**中 agent 对象增加：

| 字段 | 类型 | 说明 |
|------|------|------|
| `avatar_url` | string | 主头像 URL（无头像时不存在此字段） |
| `images` | object | 图片分组 |
| `images.avatar` | array | 头像列表（含 `id`, `url`, `is_primary`, `mime_type`, `file_size_bytes` 等） |
| `images.design` | array | 人设图列表（结构同头像） |

```json
// agent 详情中的 images 字段示例
{
  "agent": {
    "id": "agent_1",
    "avatar_url": "/api/worldbuilding/images/img_abc",
    "images": {
      "avatar": [
        {"id": "img_abc", "url": "/api/worldbuilding/images/img_abc", "is_primary": true, "mime_type": "image/png", "file_size_bytes": 204800}
      ],
      "design": [
        {"id": "img_def", "url": "/api/worldbuilding/images/img_def", "is_primary": false, "mime_type": "image/jpeg", "file_size_bytes": 1048576}
      ]
    }
  }
}
...
|------|------|------|
| `ok` | boolean | 操作是否成功 |
| `current_arc` | object\|null | 当前故事弧线（id, title, purpose, status, updated_at），仅当 current_chapter 有关联 arc 时存在 |
| `current_chapter` | object\|null | 当前章节（首个非 completed/revised 状态的章节，若全部已完成则取最后一个） |
...
| `current_scene` | object\|null | 当前场景（首个 writing/draft 状态的场景，若无则取第一个） |
...
| `secrets[].title` | string | 秘密标题（取 holder_id，若为空则取 public_version） |
...
| `world_time` | object | 世界时间（固定值 day=1, period=0，label 为 UTF-8 编码中文） |
| `world_time.day` | number | 天数，固定 1 |
| `world_time.period` | number | 时段（0=晨），固定 0 |
| `world_time.label` | string | 时间标签，固定 `"Day 1 Dawn"` |
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `status` | string | 否 | 过滤：`"outline"` / `"drafting"` / `"completed"` / `"revised"`，非法值抛异常 |
...
| `status` | string | 否 | 按状态过滤：`"draft"` / `"writing"` / `"completed"`，非法值抛异常 |
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `status` | string | 否 | 过滤：`"open"` / `"paid"` / `"abandoned"`，非法值抛异常 |
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `status` | string | 否 | 过滤：`"active"` / `"exposed"` / `"abandoned"`，非法值抛异常 |
...
以下接口在 `webui/src/api/client.ts` 中有调用代码，但后端完全未实现（无路由、无 handler、无对应 service 方法），调用返回 404：

| 方法 | 路径 | 前端函数 | 缺失情况 |
|------|------|----------|----------|
| `GET` | `/v1/runs/:id/audit` | `fetchRunAudit()` | 整个 C++ 代码库中 **零引用**，无路由注册、无 handler、无 service 方法。前端的 `RunAudit`/`RunAuditResponse` 类型已定义但无后端对应 |
| `GET` | `/api/worldbuilding/:wid/chapters/:cid` | `fetchChapterContent()` | 无路由注册、无 handler。后端存在 `PATCH`（更新章节）和 `GET /chapters`（列表），但缺少 GET 单个章节的接口，且 `NarrativeService` 中无 `get_chapter()` 方法 |
