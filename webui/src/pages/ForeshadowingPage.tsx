...
export default function ForeshadowingPage({ worldId }: { worldId: string }) {
  ...
  return (
    <main className={styles.workspace}>
      <ResponsivePane label="Foreshadowing" className={styles.listPane} closeOnSelect>
...
function ForeshadowingContext({ item, data }: { item: ForeshadowingItem; data: Data }) {
  const resolve = (type: 'planted' | 'paid', id?: string) => ({ type, id: id ?? '', label: data.scenes.find((scene) => scene.id === id)?.title ?? data.chapters.find((chapter) => chapter.id === id)?.title });
  const positions = [resolve('planted', item.planted_at), resolve('paid', item.paid_at)].filter((position) => position.id && position.label);
  return <div className={styles.context}>{positions.length ? <div><h3>Narrative positions</h3><div className={styles.chips}>{positions.map((position) => <span key={`${position.type}:${position.id}`}>{position.label}</span>)}</div></div> : <p>No resolvable narrative positions recorded.</p>}</div>;
}
