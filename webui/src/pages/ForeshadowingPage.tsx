...
export function deriveForeshadowingStatus(positions: {
  status?: string;
  ...
  if (
    ...
    positions.plannedPosition === null ||
...
export default function ForeshadowingPage({ worldId }: { worldId: string }) {
  ...
  useEffect(() => {
    setPayoff(selected?.pay_off_idea ?? '');
    setEditing(false);
  }, [selected?.id]);
  ...
  async function save() {
    if (
      ...
      !window.confirm('Update this foreshadowing item?')
    )
      return;
    ...
    setMutationError(null);
    try {
      await api.patchForeshadow(operationWorld, selected.id, { pay_off_idea: payoff });
  ...
  async function remove() {
    if (
      !selected ||
      pendingMutationRef.current ||
      !window.confirm('Delete this foreshadowing item?')
    )
      return;
    setMutationError(null);
    ...
    try {
      await api.deleteForeshadowing(operationWorld, id);
      if (mountedRef.current && generationRef.current === operationGeneration) {
  ...
  if (!data)
    return (
      <PageState
        ...
        loadingLabel="Loading foreshadowing"
    ...
    <main className={styles.workspace}>
      <aside className={styles.listPane}>
        <header>
          ...
          <div className={styles.tools}>
            <button aria-label="Refresh foreshadowing" onClick={resource.retry}>
              <RefreshCw />
            </button>
            <button
              aria-label="Create foreshadowing"
        ...
        {filtered.length ? (
          ...
          <p className={styles.empty}>No foreshadowing matches this filter.</p>
        )}
        <footer>
      ...
      <section className={styles.detail} aria-label="Foreshadowing detail">
        {selected ? (
          <>
            <header className={styles.detailHeader}>
              ...
              <div className={styles.actions}>
                ...
                <button
                  aria-label="Delete foreshadowing"
                  disabled={Boolean(pendingMutation)}
                  onClick={remove}
                >
                  <Trash2 /> Delete
            ...
            <div className={styles.cards}>
              ...
              <Fact label="Hint level" value={selected.hint_level} />
              <Fact label="Planted at" value={selected.planted_at} />
              <Fact label="Paid at" value={selected.paid_at} />
            </div>
            <section className={styles.block}>
              ...
              {editing ? (
                <>
                  <textarea
                    aria-label="Payoff idea"
                    disabled={Boolean(pendingMutation)}
                    value={payoff}
                    onChange={(event) => setPayoff(event.target.value)}
                  />
                  <button
                ...
                <p>{selected.pay_off_idea || 'No payoff idea recorded.'}</p>
              )}
            </section>
          ...
          <Pick
            icon={<Sparkles />}
            title="Select foreshadowing"
            text="Choose a thread to inspect its recorded details."
          />
        )}
      </section>
      <aside className={styles.contextPane}>
        <h2>Real context</h2>
        {selected ? (
          <ForeshadowingContext item={selected} data={data} />
        ) : (
          <p>Select a thread to see resolved narrative positions.</p>
        )}
      </aside>
      {createOpen ? (
...
function Pick({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
...
function ForeshadowingContext({ item, data }: { item: ForeshadowingItem; data: Data }) {
  const resolve = (type: 'planted' | 'paid', id?: string) => {
    if (!id) return null;
    const label =
      data.scenes.find((scene) => scene.id === id)?.title ??
      data.chapters.find((chapter) => chapter.id === id)?.title;
    return label ? { type, id, label } : null;
  };
  const positions = [resolve('planted', item.planted_at), resolve('paid', item.paid_at)].filter(
    Boolean,
  ) as { type: 'planted' | 'paid'; id: string; label: string }[];
  return (
    <div className={styles.context}>
      {positions.length ? (
        <div>
          <h3>Narrative positions</h3>
          <div className={styles.chips}>
            {positions.map((position) => (
              <span key={`${position.type}:${position.id}`}>{position.label}</span>
            ))}
          </div>
        </div>
      ) : (
        <p>No resolvable narrative positions recorded.</p>
      )}
    </div>
  );
}
