...
export default function SecretsPage({ worldId }: { worldId: string }) {
  ...
  return <main className={styles.workspace}>
    <ResponsivePane label="Secrets" className={styles.listPane} closeOnSelect><header><div><span>Knowledge boundaries</span><h1>Secrets</h1></div><div className={styles.tools}><button aria-label="Refresh secrets" onClick={resource.retry}><RefreshCw /></button><button aria-label="Create secret" disabled={Boolean(pendingMutation)} onClick={() => setCreateOpen(true)}><Plus /></button></div></header>
      ...
      <footer>{filtered.length} {filtered.length === 1 ? 'secret' : 'secrets'}</footer></ResponsivePane>
